...
bool maxiAtomBook::loadMPTKXmlBook(string filename, maxiAtomBook &book) {
	bool ok;
        ...
        ofXml doc;
        doc.load(ofToDataPath(filename));

    
        doc.setTo("book");
        map<string,string> attrm =    doc.getAttributes();
    for (std::map<string,string>::iterator it=attrm.begin(); it!=attrm.end(); ++it)
    {std::cout << it->first << " => " << it->second << '\n';}
//    cout<<attrm["nAtom"]<<endl;
    
    
        int nAtoms = ofToInt(attrm["nAtom"]);
        book.numSamples = ofToInt(attrm["numSamples"]);
        book.sampleRate = ofToInt(attrm["sampleRate"]);
        cout << book.numSamples << ", " << book.sampleRate << endl;
        int numa = doc.getNumChildren();
        doc.setToChild(0);
        for (int i = 0 ; i <numa ;i++){
            ...
            doc.setTo("support");
            doc.setTo("p");
            newAtom->position = doc.getIntValue();
//            cout<<doc.getIntValue()<<endl;
            doc.setTo("../l");
            newAtom->length = doc.getIntValue();
            doc.setTo("../../par[@type='amp']");
            
            newAtom->amp = doc.getFloatValue();
//            cout<<doc.getFloatValue()<<endl;
            doc.setTo("../par[@type='freq']");
            newAtom->frequency = doc.getFloatValue();
            
            doc.setTo("../par[@type='phase']");
            newAtom->phase= doc.getFloatValue();
