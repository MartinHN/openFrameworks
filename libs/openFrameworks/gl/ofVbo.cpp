...
void ofVbo::updateOneVertexData(const ofVec3f * vert, int idx) {
	if(positionAttribute.getId()!=0) {
        glBindBuffer(GL_ARRAY_BUFFER, positionAttribute.getId());
		glBufferSubData(GL_ARRAY_BUFFER, idx*positionAttribute.stride,positionAttribute.stride, &vert[0].x);
...
void ofVbo::updateOneColorData(const ofFloatColor * color, int idx) {
	if(colorAttribute.getId()!=0) {
		glBindBuffer(GL_ARRAY_BUFFER, colorAttribute.getId());
		glBufferSubData(GL_ARRAY_BUFFER, idx*colorAttribute.getId(), colorAttribute.stride, &color[0].r);
		glBindBuffer(GL_ARRAY_BUFFER, 0);
	}
}
